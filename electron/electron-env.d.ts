...
interface Window {
	electronAPI: {
		...
		// ── Claude CLI ──────────────────────────────────────────────────
		claudeStatus: () => Promise<{ available: boolean; version?: string; reason?: string }>;
		claudeSend: (prompt: string) => void;
		claudeCancel: () => void;
		transcribeAudio: (
			url: string,
			name: string,
		...
		installVoice?: () => Promise<{ installed: boolean; error?: string }>;
		onVoiceProgress?: (callback: (ratio: number) => void) => () => void;
		speak?: (
			text: string,
		...
		onRecordBarCommand?: (callback: (command: string) => void) => () => void;
		setRecordBarVisible?: (visible: boolean) => void;
		pushRecordBarState?: (state: unknown) => void;
		onDownloadDone?: (
			callback: (info: { filename: string; path: string; state: string }) => void,
		) => () => void;
		getPendingCursorTelemetry?: () => Promise<{
			success: boolean;
			samples: Array<{
				timeMs: number;
				cx: number;
