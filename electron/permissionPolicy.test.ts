...
describe("capture window types", () => {
	const EDITOR_URL = "http://localhost:5173/?windowType=editor-next";

	it("trusts the editor window, which is where Rendr records from", () => {
		expect(
			shouldGrantDisplayCapture(
				{
					isTrustedCaptureWindow: true,
					isMainFrame: true,
					currentDocumentUrl: EDITOR_URL,
					securityOrigin: "http://localhost:5173",
					videoRequested: true,
				},
				TRUSTED_DOCUMENT_BASE_URLS,
	...
	it("still refuses a window type that isn't a capture surface", () => {
		for (const type of ["update-toast", "source-selector", "countdown"]) {
			expect(
				shouldGrantDisplayCapture(
					{
						isTrustedCaptureWindow: true,
						isMainFrame: true,
						currentDocumentUrl: `http://localhost:5173/?windowType=${type}`,
						securityOrigin: "http://localhost:5173",
						videoRequested: true,
					},
					TRUSTED_DOCUMENT_BASE_URLS,
	...
	it("still refuses a second query parameter alongside a valid window type", () => {
		expect(
			shouldGrantDisplayCapture(
				{
					isTrustedCaptureWindow: true,
					isMainFrame: true,
					currentDocumentUrl: `${EDITOR_URL}&debug=1`,
					securityOrigin: "http://localhost:5173",
					videoRequested: true,
				},
				TRUSTED_DOCUMENT_BASE_URLS,
