/**
 * The window types allowed to capture the screen.
 *
 * Recordly captured only from its HUD. Rendr also captures from its own editor
 * — the Record button, the Record panel, and the agent's start_recording all
 * live there — so `editor-next` belongs here or capture is refused with a bare
 * "Permission denied". Recordly's legacy `editor` window never captured and is
 * deliberately absent.
 *
 * The rule this enforces is unchanged: the document URL must carry exactly one
 * query parameter, `windowType`, and its value must name one of Rendr's own
 * capture surfaces. A URL with a second parameter, a repeated parameter, or any
 * other value is still refused.
 */
const CAPTURE_WINDOW_TYPES = new Set(["hud-overlay", "editor-next"]);
...
function hasExactCaptureWindowQuery(url: URL): boolean {
	...
	return (
		...
		CAPTURE_WINDOW_TYPES.has(entries[0][1])
	);
}
