...
contextBridge.exposeInMainWorld("electronAPI", {
	// ── Agent (MCP) ─────────────────────────────────────────────────────
	// The MCP server runs in the main process; the editor state it acts on lives here.
	onAgentToolCall: (
		callback: (call: { callId: string; name: string; args: Record<string, unknown> }) => void,
	) => {
		const listener = (
			_event: Electron.IpcRendererEvent,
			call: { callId: string; name: string; args: Record<string, unknown> },
		) => callback(call);
		ipcRenderer.on("rendr-agent:request", listener);
		return () => ipcRenderer.removeListener("rendr-agent:request", listener);
	},
	respondAgentToolCall: (response: { callId: string; result?: unknown; error?: string }) => {
		ipcRenderer.send("rendr-agent:response", response);
	},

	// ── Claude CLI ──────────────────────────────────────────────────────
	claudeStatus: () => ipcRenderer.invoke("rendr-claude:status"),
	claudeSend: (prompt: string) => ipcRenderer.send("rendr-claude:send", prompt),
	claudeCancel: () => ipcRenderer.send("rendr-claude:cancel"),
	transcribeAudio: async (url: string, name: string) => {
		// The renderer owns the blob; the main process only ever sees bytes.
		const bytes = await (await fetch(url)).arrayBuffer();
		return ipcRenderer.invoke("rendr-transcribe:run", bytes, name);
	},
	onClaudeEvent: (callback: (event: unknown) => void) => {
		const listener = (_event: Electron.IpcRendererEvent, payload: unknown) => callback(payload);
		ipcRenderer.on("rendr-claude:event", listener);
		return () => ipcRenderer.removeListener("rendr-claude:event", listener);
	},
	...
	onVoiceProgress: (callback: (ratio: number) => void) => {
		const listener = (_event: unknown, ratio: number) => callback(ratio);
		ipcRenderer.on("rendr-voice:progress", listener);
		return () => ipcRenderer.removeListener("rendr-voice:progress", listener);
	},
	speak: (text: string, options?: { voice?: string; speed?: number }) =>
		ipcRenderer.invoke("rendr-voice:speak", text, options),
	listVoices: () => ipcRenderer.invoke("rendr-voice:voices"),

	/** The editor pushes recording state to the floating bar. */
	onRecordBarState: (callback: (state: unknown) => void) => {
		const listener = (_event: unknown, state: unknown) => callback(state);
		ipcRenderer.on("rendr-bar:state", listener);
		return () => ipcRenderer.removeListener("rendr-bar:state", listener);
	},
	sendRecordBarCommand: (command: string) => {
		ipcRenderer.send("rendr-bar:command", command);
	},
	/** The editor listens for what the bar's buttons did. */
	onRecordBarCommand: (callback: (command: string) => void) => {
		const listener = (_event: unknown, command: string) => callback(command);
		ipcRenderer.on("rendr-bar:command-forwarded", listener);
		return () => ipcRenderer.removeListener("rendr-bar:command-forwarded", listener);
	},
	setRecordBarVisible: (visible: boolean) => {
		ipcRenderer.send("rendr-bar:visible", visible);
	},
	pushRecordBarState: (state: unknown) => {
		ipcRenderer.send("rendr-bar:push-state", state);
	},

	/** Where a finished download actually landed, so an export can report it. */
	onDownloadDone: (
		callback: (info: { filename: string; path: string; state: string }) => void,
	) => {
		const listener = (
			_event: unknown,
			info: { filename: string; path: string; state: string },
		) => callback(info);
		ipcRenderer.on("rendr-download:done", listener);
		return () => ipcRenderer.removeListener("rendr-download:done", listener);
	},
	getPendingCursorTelemetry: () => {
		return ipcRenderer.invoke("get-pending-cursor-telemetry") as Promise<{
			success: boolean;
