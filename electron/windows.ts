...
/**
 * The icon the OS shows for a window — the taskbar, alt-tab, and the window
 * chrome on Windows and Linux.
 *
 * Rendr's own mark first. The upstream app-icons/ PNGs are Recordly's, so
 * falling straight to them is why the taskbar showed a different logo from the
 * one in the app. build/icon.png is rasterised from Rendr's own SVG.
 */
function resolveWindowIconPath(): string {
	const rendrIcon = path.join(APP_ROOT, "build", "icon.png");
	if (fs.existsSync(rendrIcon)) return rendrIcon;
	return path.join(process.env.VITE_PUBLIC || RENDERER_DIST, "app-icons", WINDOW_ICON_FILENAME);
}
...
function getEditorWindowQuery(): Record<string, string> {
	// The Palmier-derived editor is Rendr's editor. Recordly's original is still
	// reachable at ?windowType=editor for comparison while the port settles.
	const query: Record<string, string> = {
		windowType: process.env.RENDR_LEGACY_EDITOR ? "editor" : "editor-next",
	};
...
// ── Rendr's floating record bar ───────────────────────────────────────

let rendrBarWindow: BrowserWindow | null = null;

const RENDR_BAR_WIDTH = 268;
const RENDR_BAR_HEIGHT = 52;
const RENDR_BAR_BOTTOM_MARGIN = 28;

export function getRendrBarWindow(): BrowserWindow | null {
	return rendrBarWindow && !rendrBarWindow.isDestroyed() ? rendrBarWindow : null;
}

/**
 * The floating stop/pause bar shown while Rendr is capturing.
 *
 * It has to be its own window rather than a corner of the editor: a bar drawn
 * inside the editor is part of the screen, so recording the whole screen
 * records the bar. A separate window can be marked content-protected, which is
 * what keeps it out of the capture — the same thing Recordly's HUD does.
 */
export function createRendrBarWindow(): BrowserWindow {
	const existing = getRendrBarWindow();
	if (existing) {
	...
	const win = new BrowserWindow({
		width: RENDR_BAR_WIDTH,
		height: RENDR_BAR_HEIGHT,
		x: Math.round(x + (width - RENDR_BAR_WIDTH) / 2),
		y: Math.round(y + height - RENDR_BAR_HEIGHT - RENDR_BAR_BOTTOM_MARGIN),
		frame: false,
		transparent: true,
		backgroundColor: "#00000000",
		resizable: false,
		...
		hasShadow: false,
		show: false,
		// Clickable — it is the stop button — but it must never steal focus from
	...
	// The whole point: excluded from every screen capture that respects it.
	if (isHudOverlayCaptureProtectionSupported()) {
		win.setContentProtection(true);
	}
	// Above full-screen apps, so it doesn't vanish behind what's being recorded.
	...
	win.on("closed", () => {
		if (rendrBarWindow === win) rendrBarWindow = null;
	});

	if (VITE_DEV_SERVER_URL) {
		win.loadURL(`${VITE_DEV_SERVER_URL}?windowType=rendr-bar`);
	} else {
		win.loadFile(path.join(RENDERER_DIST, "index.html"), {
			query: { windowType: "rendr-bar" },
		});
	}
...
export function closeRendrBarWindow(): void {
	const win = getRendrBarWindow();
	if (win) win.close();
	rendrBarWindow = null;
}
