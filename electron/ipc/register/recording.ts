...
export function registerRecordingHandlers(
	...
	/**
	 * The samples collected by the run that just ended.
	 *
	 * `get-cursor-telemetry` reads a sidecar keyed by the video's path, which
	 * assumes the recorder wrote a file. Rendr's editor records into a Blob and
	 * has no path to key on, so it needs the samples themselves — the same ones
	 * `snapshotCursorTelemetryForPersistence` just gathered.
	 */
	ipcMain.handle("get-pending-cursor-telemetry", () => {
