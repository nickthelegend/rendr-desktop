...
// Adapted from Palmier Pro (GPL-3.0), mcpb/server/index.js. See NOTICE.md.
const http = require('node:http');

const URL_BASE = 'http://127.0.0.1:19790/mcp';
...
const log = (...a) => console.error('[rendr-shim]', ...a);
...
process.stdin.on('data', (chunk) => {
  ...
  while ((nl = stdinBuf.indexOf('\n')) >= 0) {
    ...
    handleClientMessage(msg).catch((err) => {
      ...
      if (msg.id !== undefined) {
        writeOut({ jsonrpc: '2.0', id: msg.id, error: { code: -32603, message: `Rendr unreachable: ${err.message}` } });
      }
    });
  }
});
