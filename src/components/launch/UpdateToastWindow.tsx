...
function getToastTitle(payload: UpdateToastPayload) {
	...
	switch (payload.phase) {
		case "available":
			return `Rendr ${payload.version} is available`;
		case "downloading":
			return `Installing Rendr ${payload.version}`;
		case "ready":
			return `Rendr ${payload.version} is ready`;
		case "error":
			return payload.primaryAction === "retry-check"
				? "Could not check for updates"
				: `Rendr ${payload.version} needs attention`;
	}
}
