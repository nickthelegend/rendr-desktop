...
function LaunchWindowContent() {
	...
	// Agent surface: this window hosts the recorder half of the MCP tool set.
	// selectedSource is declared further down, so mirror it through a ref the
	// getter can read at call time.
	useAgentTools({
		recorder: () => ({
			recording,
			paused,
			...
			setSystemAudioEnabled,
			setCountdownDelay,
			selectedSourceName: selectedSourceRef.current,
	...
	selectedSourceRef.current = selectedSource ?? null;
